...
fig = go.Figure(go.Indicator(mode="gauge+number",value=risk,title="Risk %",gauge={"axis":{"range":[0,100]},"steps":[{"range":[0,30],"color":"lightgreen"},{"range":[30,70],"color":"yellow"},{"range":[70,100],"color":"lightcoral"}]}))
st.plotly_chart(fig)

df = pd.DataFrame({"Metric":["CPU","Memory","Network","Power"],"Value":[f"{cpu}%",f"{mem}%",f"{net}",f"{pow}"]})
st.dataframe(df)
